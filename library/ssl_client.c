                                        ...
                                        size_t *out_len )
{
    ...
    *out_len = 0;
     ...
     * opaque Random[32];
     *
     * The random bytes have prepared by ssl_prepare_client_hello() into
    ...
    *p++ = (unsigned char)ssl->session_negotiate->id_len;
    ...
    if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
    {
        ...
        *p++ = cookie_len;
        if( cookie_len > 0 )
        {
            memcpy( p, ssl->handshake->verify_cookie, cookie_len );
            p += cookie_len;
        }
    }
...
static int ssl_finalize_client_hello( mbedtls_ssl_context *ssl )
{
