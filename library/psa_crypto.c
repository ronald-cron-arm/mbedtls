    ...
    size_t *key_buffer_length, size_t *bits )
{
    ...
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_PUBLIC_KEY)
        ...
        {
            return( psa_crypto_ecp_import_key( attributes,
                                               data, data_length,
    ...
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_PUBLIC_KEY)
        ...
        {
            return( psa_crypto_rsa_import_key( attributes,
                                               data, data_length,
                                               key_buffer, key_buffer_size,
                                               key_buffer_length,
                                               bits ) );
...
#if defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_KEY_PAIR) || \
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_PUBLIC_KEY)
            return( psa_crypto_rsa_export_public_key(
                        attributes,
                        key_buffer, key_buffer_size,
                        data, data_size, data_length ) );
#else
...
#if defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR) || \
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_PUBLIC_KEY)
            return( psa_crypto_ecp_export_public_key(
                        attributes,
                        key_buffer, key_buffer_size,
                        data, data_size, data_length ) );
#else
    ...
    uint8_t *signature, size_t signature_size, size_t *signature_length )
{
    ...
    {
        return( psa_crypto_rsa_sign_hash(
                    attributes,
                    key_buffer, key_buffer_size,
    ...
    const uint8_t *signature, size_t signature_length )
{
    ...
    {
        return( psa_crypto_rsa_verify_hash(
                    attributes,
                    key_buffer, key_buffer_size,
                    alg, hash, hash_length,
            ...
            if( mbedtls_ecp_is_zero( &ecp->Q ) )
            {
                MBEDTLS_MPI_CHK(
                    mbedtls_ecp_mul( &ecp->grp, &ecp->Q, &ecp->d, &ecp->grp.G,
                                     mbedtls_ctr_drbg_random, psa_get_ctr_drbg_context( ) ) );
            }
    ...
    uint8_t *key_buffer, size_t key_buffer_size, size_t *key_buffer_length )
{
    ...
    {
        return( psa_crypto_rsa_generate_key( attributes,
    ...
    if ( PSA_KEY_TYPE_IS_ECC( type ) && PSA_KEY_TYPE_IS_KEY_PAIR( type ) )
    {
        return( psa_crypto_ecp_generate_key( attributes,
                    key_buffer, key_buffer_size, key_buffer_length ) );
    }
    else
#endif /* defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR) */
