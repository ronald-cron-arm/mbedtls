...
psa_status_t psa_validate_key_id(
    mbedtls_svc_key_id_t key, int vendor_ok, int transient_ok )
{
    ...
    if( vendor_ok &&
        ...
        ( key_id < PSA_KEY_ID_TRANSIENT_MIN ) )
        return( PSA_SUCCESS );

    if( transient_ok &&
        ( PSA_KEY_ID_TRANSIENT_MIN <= key_id ) &&
 ...
 *  any storage.
 *
 *  For transient key identifiers, only one key slot is queried as a volatile
 *  key with transient key identifier key_id can only be stored in slot of
 *  index ( key_id - #PSA_KEY_ID_TRANSIENT_MIN ).
 *
    ...
    mbedtls_svc_key_id_t key, psa_key_slot_t **p_slot )
{
    ...
    if( psa_key_id_is_transient( key_id ) )
    {
                                     ...
                                     psa_key_slot_t **p_slot )
{
    ...
    for( slot_idx = PSA_KEY_SLOT_COUNT; slot_idx > 0; slot_idx-- )
    {
        *p_slot = &global_data.key_slots[ slot_idx - 1 ];
        ...
        {
            *transient_key_id = PSA_KEY_ID_TRANSIENT_MIN +
                               ...
                               psa_key_slot_t **p_slot )
{
    ...
    *p_slot = NULL;
    ...
    status = psa_get_empty_key_slot( &transient_key_id, p_slot );
    if( status != PSA_SUCCESS )
        return( status );
