...
psa_status_t psa_crypto_rsa_load_representation(
    ...
    size_t *key_buffer_length, size_t *bits )
{
    ...
    /* Parse input */
    status = psa_crypto_rsa_load_representation( attributes->core.type,
                                                 data,
                                                 data_length,
                                                 &rsa );
    if( status != PSA_SUCCESS )
    ...
    *bits = (psa_key_bits_t) PSA_BYTES_TO_BITS( mbedtls_rsa_get_len( rsa ) );
    ...
    status = psa_crypto_rsa_export_key( attributes->core.type,
                                        rsa,
                                        key_buffer,
                                        key_buffer_size,
                                        key_buffer_length );
...
psa_status_t psa_crypto_rsa_export_key( psa_key_type_t type,
    ...
    uint8_t *data, size_t data_size, size_t *data_length )
{
    ...
    status = psa_crypto_rsa_export_key( PSA_KEY_TYPE_RSA_PUBLIC_KEY, rsa,
...
psa_status_t psa_crypto_rsa_import_key(
    const psa_key_attributes_t *attributes,
    const uint8_t *data, size_t data_length,
    uint8_t *key_buffer, size_t key_buffer_size,
    size_t *key_buffer_length, size_t *bits )
...
psa_status_t psa_crypto_rsa_export_public_key(
    const psa_key_attributes_t *attributes,
    const uint8_t *key_buffer, size_t key_buffer_size,
    uint8_t *data, size_t data_size, size_t *data_length )
{
...
#if defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_KEY_PAIR)
psa_status_t psa_crypto_rsa_generate_key(
    const psa_key_attributes_t *attributes,
    uint8_t *key_buffer, size_t key_buffer_size, size_t *key_buffer_length )
{
...
psa_status_t psa_crypto_rsa_sign_hash(
    const psa_key_attributes_t *attributes,
    const uint8_t *key_buffer, size_t key_buffer_size,
    psa_algorithm_t alg, const uint8_t *hash, size_t hash_length,
    uint8_t *signature, size_t signature_size, size_t *signature_length )
...
psa_status_t psa_crypto_rsa_verify_hash(
    const psa_key_attributes_t *attributes,
    const uint8_t *key_buffer, size_t key_buffer_size,
    psa_algorithm_t alg, const uint8_t *hash, size_t hash_length,
    const uint8_t *signature, size_t signature_length )
