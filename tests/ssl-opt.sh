...
run_test    "TLS 1.2: m->O, server forces TLS 1.2 and rsa_pss_rsae_sha256" \
            "$O_NEXT_SRV_NO_CERT -cert $DATA_FILES_PATH/server2-sha256.crt \
                                 -key $DATA_FILES_PATH/server2.key \
...
requires_config_enabled PSA_WANT_ALG_RSA_PSS
requires_config_enabled PSA_WANT_ALG_SHA_256
run_test    "TLS 1.2: m->G, server forces TLS 1.2 and rsa_pss_rsae_sha256" \
            "$G_NEXT_SRV_NO_CERT --x509certfile $DATA_FILES_PATH/server2-sha256.crt \
                --x509key $DATA_FILES_PATH/server2.key --disable-client-cert \
